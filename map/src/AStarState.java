...
public class AStarState
{
    ...
    public Waypoint getMinOpenWaypoint()
    {
        ...
        while (i.hasNext())
        {
          ...
          float waypoint_total_cost = waypoint.getTotalCost();
            ...
            // если общая стоимость для тек точки ниже, чем сохр стоимость для лучшей точки - заменить
    ...
    public boolean addOpenWaypoint(Waypoint newWP)
    {
        ...
        // проверяет есть ли открытая точка на новом местоположении
        if (open_waypoints.containsKey(location))
        {
            // если у новой точкиесть открытая точка, проверяет меньше л стоимость предыдущей точки
            Waypoint current_waypoint = open_waypoints.get(location);
            if (newWP.getPreviousCost() < current_waypoint.getPreviousCost())
            {
                // если значение меньше заменяет старую точку
                open_waypoints.put(location, newWP);
                return true;
            }
            // если не меньше возвращает фолс
            return false;
        }
        open_waypoints.put(location, newWP);
        return true;
